...
Set up WAL archiving on the EDB Postgres Advanced Server server. WAL archiving prepares Postgresql/EDB Postgres Advanced Server database servers for backup and recovery operations and is a precondition for any backup and recovery tool to work with the database server.
...
!!! Note 
If you haven't already set `archive_mode=on`, when you run your first backup on the database, Cohesity sets `archive_mode=on` in `postgresql.conf` and restarts the database. 
!!!